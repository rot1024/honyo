import { app, clipboard, Notification, Menu, Tray, nativeImage, BrowserWindow, ipcMain } from 'electron';
...
function getAIProvider(modelId: string): any {
...
async function translateText(text: string, primaryLanguage: string, secondaryLanguage: string): Promise<string> {
  try {
    // Pre-check API key
